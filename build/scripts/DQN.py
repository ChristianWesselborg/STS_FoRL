...
class ActorAgent:
        ...
        if np.random.rand() <= self.epsilon:
            ...
            for i in range(len(masks)):
                if masks[i]:
                    ...
                    index.append(i)
            softmaxdist = np.array(softmaxdist)
    ...
    def replay(self, batch_size):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = tf.math.minimum(ratio * advantage, np.clip(ratio, a_min=1 - LOSS_CLIPPING, a_max=1 + LOSS_CLIPPING) * advantage)
...
def takeNextStateUpdateNetwork(old_state,card_state, enemy_state, pot_state, action, reward, next_state, next_state_card, next_state_enemy, next_state_pot, done, masks_next, e):
    ...
    actor.remember([card_state, enemy_state, pot_state, old_state], action, reward, [next_state_card, next_state_enemy, next_state_pot, next_state], done, masks_next)
    if(done == True):
